...
`;
    ...
    const registrations = sorted
        .map(({ id, hasLoader, hasClientLoader, hasFallback }, index) => {
            if (hasLoader || hasClientLoader || hasFallback) {
                ...
                return `    targetRegistry.registerImporter('${id}', staticRegistryImporters[${index}]);`;
            }
        })
        .join('\n');
    ...
    return {
        ...
        load(id) {
            ...
            if (!isProductionBuild) {
                return `export default ${JSON.stringify(createEmptyPageDesignerPreloadManifest())};`;
            }
