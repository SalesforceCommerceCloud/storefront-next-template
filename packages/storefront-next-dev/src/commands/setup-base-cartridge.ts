 ...
 */
export default class SetupBaseCartridge extends OAuthCommand<typeof SetupBaseCartridge> {
    static description = 'Register the required SLAS scopes for the base cartridge on a client';

    static examples = [
        '<%= config.bin %> <%= command.id %> --slas-client-id my-slas-client-id',
        '<%= config.bin %> <%= command.id %> --slas-client-id my-slas-client-id --tenant-id my_tenant_001',
    ];
    ...
    async run(): Promise<void> {
        ...
        if (!shortCode) {
            this.error(
                'SCAPI short code required. Provide --short-code, set SFCC_SHORTCODE, or configure it in dw.json.'
            );
        }
        if (!tenantId) {
            this.error('Tenant ID required. Provide --tenant-id, set SFCC_TENANT_ID, or configure it in dw.json.');
        }
        ...
        } = await client.GET('/tenants/{tenantId}/clients/{clientId}', {
            params: { path: { tenantId, clientId: targetClientId } },
        });

        if (getError || !existing) {
            const body = await getResponse.text().catch(() => '');
            this.error(
                `Failed to fetch SLAS client: ${getResponse.status} ${getResponse.statusText}${body ? `\n${body}` : ''}`
            );
        }

        // SLAS Admin API can return string fields as a single delimited string rather
        // than an array. Split on pipe or whitespace to normalise before PUT.
        const splitDelimited = (value: string | string[] | undefined): string[] =>
            ([] as string[]).concat(value ?? []).flatMap((u) => u.split(/[|\s]+/).filter(Boolean));

        const currentScopes = splitDelimited(existing.scopes as string | string[] | undefined);

        if (currentScopes.includes(SFNEXT_NOTIFY_SCOPE)) {
            this.log(
                `Scope "${SFNEXT_NOTIFY_SCOPE}" is already registered on client "${targetClientId}". Nothing to do.`
            );
            return;
        }
        ...
        const callbackUri = existing.callbackUri != null ? splitDelimited(existing.callbackUri) : undefined;
