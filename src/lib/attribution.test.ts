...
describe('buildSanitizedReferrer', () => {
    test('returns empty string when there is no referrer', () => {
        ...
        expect(buildSanitizedReferrer(null, 'shop.example.com')).toBe('');
    });
    ...
    test('drops a same-origin referrer (internal navigation is not attribution)', () => {
        expect(buildSanitizedReferrer('https://shop.example.com/category/shoes', 'shop.example.com')).toBe('');
    });

    test('keeps the origin and path for an external referrer with no attribution params', () => {
        expect(buildSanitizedReferrer('https://blog.com/post', 'shop.example.com')).toBe('https://blog.com/post');
    });

    test('normalizes a bare origin to a trailing-slash path', () => {
        // `new URL('https://blog.com').pathname` is '/', so a path is always present.
        expect(buildSanitizedReferrer('https://blog.com', 'shop.example.com')).toBe('https://blog.com/');
    });

    test('keeps the path verbatim (the referrer path is part of the attribution signal)', () => {
        // The path is retained (W-23493124 — it is signal ECOM emits as sessionReferrer). PII
        // minimization is by the query allowlist, not by dropping the path; free-text query params
        // are still dropped below.
        expect(
            buildSanitizedReferrer('https://blog.com/campaigns/spring-sale?utm_source=news', 'shop.example.com')
    ...
    test('drops the fragment', () => {
        expect(buildSanitizedReferrer('https://blog.com/post?utm_source=news#section', 'shop.example.com')).toBe(
            'https://blog.com/post?utm_source=news'
        );
    });

    test('drops credentials (userinfo) from the referrer but keeps the path', () => {
        expect(buildSanitizedReferrer('https://user:pass@blog.com/post?utm_source=news', 'shop.example.com')).toBe(
            'https://blog.com/post?utm_source=news'
        );
    });
...
describe('buildAttributionValue', () => {
    ...
    test('returns empty string when there is nothing to attribute', () => {
        expect(buildAttributionValue({ searchParams: params(''), requestHostname: 'shop.example.com' })).toBe('');
    ...
    test('ignores non-allowlisted landing params', () => {
        const value = buildAttributionValue({
            searchParams: params('utm_source=google&email=shopper%40example.com&sessionToken=secret'),
            requestHostname: 'shop.example.com',
        });
    ...
    test('drops ref first when the value exceeds the length cap', () => {
        // ref is dropped before the landing params (it is the longest component and the current
        // URL's own params are the primary signal).
    ...
    test('never exceeds the length cap even when a single landing param is huge', () => {
        const value = buildAttributionValue({
            searchParams: params(`utm_source=${'x'.repeat(MAX_ATTRIBUTION_COOKIE_VALUE_LENGTH + 500)}`),
            requestHostname: 'shop.example.com',
        });
        expect(value.length).toBeLessThanOrEqual(MAX_ATTRIBUTION_COOKIE_VALUE_LENGTH);
    });
    ...
    test('percent-encodes hostile characters so no cookie/header separators survive', () => {
        ...
        // The raw separators that would let a value break out of the cookie or inject a header must
        // not appear unencoded in the serialized value.
...
describe('serializeAttributionCookie', () => {
    ...
    test('ignores an invalid cookieDomain and falls back to a host-scoped cookie', () => {
        // Wildcards and separators are rejected by the shared validation; a stray `;` could
        // otherwise be read as an extra cookie attribute.
        const cookie = serializeAttributionCookie('utm_source=google', {
            maxAge: ATTRIBUTION_COOKIE_MAX_AGE_SECONDS,
...
describe('captureAttribution / clearAttribution (client cookie I/O)', () => {
    ...
    const campaign = (overrides: Record<string, unknown> = {}) => ({
        ...
        // jsdom's document URL is http://localhost, so a Secure cookie would be dropped by the
        // cookie jar; write a non-Secure cookie in tests so we can read it back.
        secure: false,
        ...overrides,
    });
    ...
    test('captures a sanitized referrer when there is no landing param', () => {
        ...
        expect(document.cookie).toContain('dw_attribution=ref=https%3A%2F%2Fblog.com%2Fpost%3Futm_source%3Dnews');
    });
    ...
    test('does not throw and writes nothing when capture inputs are hostile', () => {
        // Fails open: even if buildAttributionValue produced something odd, capture must not throw.
