...
- Mark a Page Designer region as [`<Region critical>`](README-PAGE-DESIGNER.md#critical-page-regions) only when its content is required for the initial HTML or is an LCP candidate. Await that page in the route loader. The production build manifest lets SSR preload and register the region's nested component modules and activate their styles before the initial shell, while component loader data keeps its granular Suspense boundaries. Do not mark below-the-fold or broad catch-all regions as critical: doing so delays the shell and consumes the preload budget without improving the initial viewport.
